...
https://semfundo.app/en/

> Note: use the `/en/` URL in the Website field — Product Hunt's audience is English-speaking and should land on the English page directly.
...
The whole thing runs client-side. It uses @imgly/background-removal, which loads a segmentation model through ONNX Runtime Web (WASM) and runs it right in your browser tab — no backend, no upload, no queue. The first time you use it, a ~40MB model downloads and caches locally; after that, each image processes in seconds, fully offline. Since there's no server in the loop, I can't see your images even if I wanted to — and it's open source (AGPL), so you don't have to take my word for it.
